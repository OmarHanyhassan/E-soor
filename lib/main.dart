										...
										child: Column(
											children: <Widget>[
												UserAccountsDrawerHeader(
													...
													currentAccountPicture: Image.network('https://placekitten.com/500/500'),
...
// ignore: must_call_super
	...
	void start() => speechRecognition.listen(locale: 'en_US').then(
		(result) => print("Started listening => result $result")
	);
	
	void cancel() => speechRecognition.cancel().then(
		(result) => setState(
			() => _isListening = result
		)
	);
	
	void stop() => speechRecognition.stop().then(
			(result) => setState(
				() => _isListening = result
		)
	);
...
void showSearchPage(BuildContext context, SearchDelegate search, String transcription) async {
	final String selected = await showSearch(context: context, delegate: search, query: transcription);
